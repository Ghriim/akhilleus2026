...
final class RemoveExerciseSetValidatorTest extends TestCase
{
    private RemoveExerciseSetValidator $validator;
    private PlayerDataModel $player;

    protected function setUp(): void
    {
        $this->validator = new RemoveExerciseSetValidator($this->createMock(LoggedPlayerResolverInterface::class));
    ...
    public function testItPassesForAnInProgressWorkout(): void
    {
        ...
        $this->validator->validate($this->player, new RemoveExerciseSetDataInput($set->id), $set);
    ...
    {
        $set = self::buildSet(self::buildWorkout($this->player, WorkoutStatusRegistry::COMPLETED));

        try {
            $this->validator->validate($this->player, new RemoveExerciseSetDataInput($set->id), $set);
            self::fail('Expected ValidationException');
        } catch (ValidationException $e) {
            self::assertSame(RemoveExerciseSetValidator::ERROR_CODE, $e->errorCode);
