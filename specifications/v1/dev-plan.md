...
- **Last completed step**: **3.7 — Admin LevelingConfig (`xpPerWorkoutMinute`)**. Backend: `Get`/`Update LevelingConfigUseCase` under `UseCase/Admin/Leveling/LevelingConfig/` (Get extends `AbstractPublicUseCase` + empty `GetLevelingConfigDataInput`; Update extends `AbstractLoggedAdminUseCase`), `UpdateLevelingConfigValidator` (`≥ 50`, const `MIN_XP_PER_WORKOUT_MINUTE`, error code `LEVELING_CONFIG_VALIDATION_FAILED`, no extra gateway — no cross-row rule), `LevelingConfigDataOutput {id, xpPerWorkoutMinute}`, `LevelingConfigAdminController` (`GET`/`PUT /api/admin/leveling-config`). Singleton already seeded by `Version20260613130000` (3.5) → no migration this step. Frontend: `frontend/admin/src/features/levelBrackets/levelingConfig.ts` (types+api+hooks) + `LevelingConfigCard.tsx` mounted atop `LevelBracketListPage` (no new route/menu). Tests: 3 integration + 5 validator unit. `composer cs`/`stan` green; in-container phpunit **414 tests / 826 assertions**; admin `typecheck`/`lint`/`build` green. **Uncommitted** (3.1→3.7 + the 3.6 side fixes on disk).
- **Next pending step**: **3.8 — Player XP journal**. `UseCase/Player/Leveling/EarnedExperience/ListEarnedExperienceUseCase` — paginated (`{page, perPage}`, `MAX_PER_PAGE = 50`), `earnedAt DESC`, returns `EarnedExperienceJournalDataOutput {items, page, perPage, totalCount}` (each item: `id, label, amount, earnedAt(ISO), sourceType, sourceId, isLocked`). `EarnedExperiencePlayerController::journal` → `GET /api/player/leveling/journal?page=1&perPage=20`. Frontend `frontend/website/src/features/leveling/JournalPage.tsx` (paginated list, lock icon when `isLocked`), route `/leveling/journal` linked from the header dropdown. Provider methods `findAllByPlayerForJournal` + `countByPlayerForJournal` already exist (3.4); player UC integration tests use the manual-instantiation + stubbed `LoggedPlayerResolverInterface` pattern (note: website uses `components/<domain>/` not `features/` per the 2.6 deviation — confirm on disk before scaffolding the page).
