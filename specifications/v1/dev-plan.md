...
- **Last completed step**: **4.1 — Questing DataModels + Registries** (start of Phase 4). `QuestDataModel` (`quest` table) + `QuestProgressionDataModel` (`quest_progression`, implements `OwnedByPlayerInterface`, unique `(quest_id, player_id, start_date)`) + 4 registries (`QuestKind`/`QuestPeriodicity`/`QuestMetric`/`QuestProgressionStatus`, all **UPPERCASE** values). Migration `Version20260614153305` (2 tables + FKs + unique) applied dev + test. `cs`/`stan` green; suite **425 tests / 858 assertions** (unchanged — entities inert until gateways land in 4.2). **Uncommitted** (everything 3.1→4.1 + dev-env side fixes on disk). Dev-env: Symfony `--no-tls`.
- **Next pending step**: **4.2 — Gateways + Repositories + Persisters** (Questing). `QuestProviderGateway` (`findActiveAtForList(now)`, `findActiveByPeriodicityForPlayer(periodicity, now)`, `findOneByIdForAdminAction`), `QuestPersisterGateway` (CRUD triplet), `QuestProgressionProviderGateway` (`findOneByPlayerQuestPeriod(player, quest, ?startDate)`, `findAllByPlayerActiveDaily/Weekly/Monthly(player, now)`, `findAllUniqueByPlayer(player)`, `findOneByIdForPlayerAction(id, player)`), `QuestProgressionPersisterGateway` (CRUD). Repos use context-named methods (no generic finders); persisters are pass-through (no derived props). Mirror the Tracking 2.3 layout. NB the `findOneByPlayerQuestPeriod` lookup must handle the null-`startDate` (UNIQUE) case with `IS NULL` rather than `= :startDate`.
