...
- **Last completed step**: **3.6 — Admin LevelBracket CRUD (backend + frontend)**. Backend: 5 UseCases + Create/Update validators (umbrella `LEVEL_BRACKET_VALIDATION_FAILED`, named sub-codes dropped) backed by a pure `Domain/Service/Leveling/LevelCurveEvaluator`, `LevelBracketAdminController` (`/api/admin/level-brackets`), tests (5 integration + 2 validator + exhaustive evaluator). Frontend: `frontend/admin/src/features/levelBrackets/` (List with MUI X Charts curve preview, Create, Edit, Form) + routes + menu entry. `composer qa` green (**406 tests / 812 assertions**, cs ✅, stan ✅); admin `typecheck`/`lint`/`build` green. **Side fixes this session** (dev-env, not 3.6): `compose.yaml` admin `VITE_API_BASE_URL` https→http (no local CA), `frontend/admin/src/index.css` global reset added + imported in `main.tsx`, `@mui/x-charts`+MUI+emotion added to `frontend/admin/package.json`. **Uncommitted** (3.1→3.6 + side fixes on disk).
- **Next pending step**: **3.7 — Admin LevelingConfig (`xpPerWorkoutMinute`)**. `Get`/`Update LevelingConfigUseCase` (singleton via `getSingleton()` — already seeded by `Version20260613130000` in 3.5, so no seed trap here), `UpdateLevelingConfigValidator` (integer ≥ 50, error code `LEVELING_CONFIG_VALIDATION_FAILED`), `LevelingConfigAdminController` (`GET`/`PUT /api/admin/leveling-config`), and a "Global config" form in the admin (the dev-plan suggests folding it into the `levelBrackets` page area). `LevelingConfigPersisterGateway` already exposes `update` (3.4).
