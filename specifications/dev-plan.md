...
- **IDs**: ULID via `symfony/uid` everywhere.
...
### 0.1 Docker dev environment
- `compose.yaml` services: `php` (custom Dockerfile, PHP 8.4 + composer + xdebug), `database` (mysql:8.4), `mailer` (mailhog) optional.
- `docker/php/Dockerfile`: PHP 8.4 FPM-CLI, composer, opcache, intl/pdo_mysql/zip extensions, symfony-cli binary baked in.
- Bind mount the repo into the php container; expose port 8000 via `symfony serve` from inside or outside.
- `.env` additions: `DATABASE_URL=mysql://app:!ChangeMe!@database:3306/akhilleus?serverVersion=8.4`, `JWT_PASSPHRASE`, `JWT_SECRET_KEY`, `JWT_PUBLIC_KEY`, `CORS_ALLOW_ORIGIN`.

### 0.2 Composer dependencies (production)
- `doctrine/orm`, `doctrine/doctrine-bundle`, `doctrine/doctrine-migrations-bundle`
- `doctrine/doctrine-fixtures-bundle` (the "FixtureBundle" referenced in conventions)
- `lexik/jwt-authentication-bundle`
- `symfony/security-bundle`, `symfony/serializer-pack`, `symfony/validator`
- `symfony/uid` (ULIDs)
- `symfony/clock` (PSR-20 `ClockInterface` for AbstractBaseMysqlPersister)
- `nelmio/cors-bundle`
- `nelmio/api-doc-bundle` (OpenAPI for the React frontends)

### 0.3 Composer dependencies (dev)
- `phpunit/phpunit`, `symfony/test-pack`, `dama/doctrine-test-bundle` (transactional tests)
- `phpstan/phpstan`, `phpstan/phpstan-symfony`, `phpstan/phpstan-doctrine`, `phpstan/phpstan-phpunit`
- `friendsofphp/php-cs-fixer`
- `captainhook/captainhook`, `captainhook/plugin-composer`

### 0.4 Quality toolchain config
- `phpunit.xml.dist` — bootstrap, test suites for `tests/Unit` and `tests/Integration`.
- `phpstan.dist.neon` — level 8, scan `src/` and `tests/`, Symfony + Doctrine + PHPUnit extensions, baseline empty.
- `.php-cs-fixer.dist.php` — PSR-12 base + custom rules: `declare_strict_types`, `yoda_style` (all comparisons), `final_class` (warn-only — DataModels excepted), `ordered_imports`, `no_unused_imports`.
- `captainhook.json` — `pre-commit` runs in order: `php-cs-fixer fix --dry-run --diff` → `phpstan analyse --memory-limit=1G` → `phpunit --testsuite=fast`. `commit-msg` enforces conventional commit prefix (optional, drop if undesired).
- Composer scripts: `composer test`, `composer stan`, `composer cs`, `composer cs:fix`, `composer qa` (runs all three).

### 0.5 Source skeleton & abstractions
...
- `AbstractBaseMysqlPersister` — injects `EntityManagerInterface` + `ClockInterface`. Public `create($model)`, `update($model)`, `delete($model)` set `createdAt` / `updatedAt`, flush, and call protected `postCreate`/`postUpdate`/`postDelete` hooks.
- `AbstractPublicUseCase` — abstract `execute(DataInputInterface $input)`, injects `DomainValidatorInterface`.
- `AbstractLoggedUserUseCase` — same but injects `AbstractLoggedUserValidator` (which exposes `getLoggedPlayer()` etc.).
- `DomainValidatorInterface` — `validate(object $input): void` throws `ValidationException`.

### 0.6 Verification
- `docker compose up -d database` then `composer install` + `php bin/console doctrine:database:create`.
- `composer qa` runs cleanly against the empty `src/`.
- A throwaway commit triggers the captainhook pipeline.
...
### 1.1 Entities (in dependency order)
...
### 1.2 Registries (`Domain/Registry/...`)
- `Workout/WorkoutStatusRegistry` — `PLANNED`, `IN_PROGRESS`, `COMPLETED`, `CANCELED`
- `Movement/MovementTrackingFieldRegistry` — `REPETITIONS`, `WEIGHT`, `DURATION`, `DISTANCE`, `INCLINE_PERCENT`, `INCLINE_METERS`
- `PersonalBest/PersonalBestTypeRegistry` — `HIGHEST_WEIGHT`, `HIGHEST_REPS`, `HIGHEST_VOLUME_ONE_SET`, `HIGHEST_VOLUME_WORKOUT`, `HIGHEST_DURATION`, `HIGHEST_DISTANCE`, `HIGHEST_SPEED`
- `User/UserRoleRegistry` — `ROLE_PLAYER`, `ROLE_ADMIN` (Coach deferred)
...
- `AddMovementToWorkoutUseCase`, `RemoveMovementFromWorkoutUseCase`, `UpdateMovementRestDurationUseCase`, `ReorderMovementsUseCase`.
- `AddExerciseSetUseCase` (planned values), `UpdateExerciseSetPlannedUseCase`, `UpdateExerciseSetAchievedUseCase`, `RemoveExerciseSetUseCase`, `MarkExerciseSetCompletedUseCase`.
...
- `composer.json` (additions), `phpunit.xml.dist`, `phpstan.dist.neon`, `.php-cs-fixer.dist.php`, `captainhook.json`
...
| Phase | How to verify |
|---|---|
| 0 | `composer qa` green; pre-commit hook fires; `docker compose up` healthy |
