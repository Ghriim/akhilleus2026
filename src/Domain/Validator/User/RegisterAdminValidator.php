...
final readonly class RegisterAdminValidator
{
    public const string ERROR_CODE = 'REGISTER_ADMIN_VALIDATION_FAILED';
    ...
    public function validate(RegisterAdminDataInput $input): void
    {
        $violations = [];
        ...
        if (true === empty(trim($input->firstName))) {
            $violations['firstName'][] = 'First name must not be empty.';
        }
        if (true === empty(trim($input->lastName))) {
            $violations['lastName'][] = 'Last name must not be empty.';
        }
        if (true === empty(trim($input->jobTitle))) {
            $violations['jobTitle'][] = 'Job title must not be empty.';
        }
        ...
        if ([] !== $violations) {
            throw new ValidationException('Admin registration data is invalid.', $violations, self::ERROR_CODE);
        }
    }
}
