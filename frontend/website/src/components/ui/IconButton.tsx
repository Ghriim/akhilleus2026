...
export function IconButton({ label, className, children, type = 'button', ...rest }: IconButtonProps) {
  return (
    <button
      ...
      title={label}
      className={cn(
        'inline-flex h-8 w-8 items-center justify-center rounded-(--radius-md) text-(--color-text-muted)',
        'transition-colors hover:bg-(--color-surface-muted) hover:text-(--color-primary)',
